...
layui.define(['element', 'common'], function (exports) {
    ...
    Tab.prototype.refresh = function(){
        ...
        var id = $("li[lay-id="+tabIndex+"]").find('i.layui-unselect').data('id');
        var src = ELEM.contentBox.find('iframe[data-id=' + id + ']')[0].src;

        if (navigator.userAgent.indexOf('Firefox') >= 0) {
            var urljoinstr = (src.indexOf("?") == -1) ? "?" : "&";
            ELEM.contentBox.find('iframe[data-id=' + id + ']')[0].src = src+urljoinstr+(new Date()).getTime();
        }else{
    ...
    Tab.prototype.tabAdd = function (data) {
        ...
        if (tabIndex === -1) {
            ...
            globalTabIdIndex++;
            var dataHrefUrl = data.href;
            if (navigator.userAgent.indexOf('Firefox') >= 0) {
                var urljoinstr = (dataHrefUrl.indexOf("?") == -1) ? "?" : "&";
                data.href = data.href+urljoinstr+(new Date()).getTime()
            }

            var content = '<iframe style="display: none"  src="' + data.href + '" data-id="' + globalTabIdIndex + '"></iframe>';
            var title = '';
            ...
            $(ELEM.contentBox.find('iframe[data-id=' + globalTabIdIndex + ']')).on('load', function () {
                $(this).fadeIn();
                window.history.pushState({'title':data.title,'tabId':String(tabId)}, data.title,dataHrefUrl);
                if (_config.openWait) {
                    ...
                    $context.eq(0).children('ul').children('li').each(function () {
                        ...
                        $that.on('click', function () {
                            ...
                            switch (target) {
                                case 'refresh': //刷新当前
                                    var src = ELEM.contentBox.find('iframe[data-id=' + id + ']')[0].src;
                                    ELEM.contentBox.find('iframe[data-id=' + id + ']')[0].src = src+"?123456";
                                    break;
    ...
    Tab.prototype.tabChange_goto = function (id) {
        var that = this;
